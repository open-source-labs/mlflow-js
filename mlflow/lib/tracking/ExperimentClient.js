...
class ExperimentClient {
    ...
    async createExperiment(name, artifact_location, tags) {
        const { response, data } = await apiRequest(this.trackingUri, 'experiments/create', {
            method: 'POST',
            body: { name, artifact_location, tags },
        });
        if (!response.ok) {
            throw new ApiError(`Error creating experiment from tracking server: ${data.message || response.statusText}`, response.status);
        }
     ...
     * @throws {ApiError} If the API request fails
     */
    async searchExperiment(filter, max_results, page_token, order_by, view_type) {
        const { response, data } = await apiRequest(this.trackingUri, 'experiments/search', {
            method: 'POST',
            body: { filter, max_results, page_token, order_by, view_type },
        });
        if (!response.ok) {
            throw new ApiError(`Error searching for experiment from tracking server: ${data.message || response.statusText}`, response.status);
        }
        return data;
    }
    /**
    ...
    async getExperiment(experiment_id) {
        const { response, data } = await apiRequest(this.trackingUri, 'experiments/get', {
            method: 'GET',
            params: { experiment_id }
        });
        if (!response.ok) {
            throw new ApiError(`Error getting experiment from tracking server: ${data.message || response.statusText}`, response.status);
        }
    ...
    async getExperimentByName(experiment_name) {
        const { response, data } = await apiRequest(this.trackingUri, 'experiments/get-by-name', {
            method: 'GET',
            params: { experiment_name }
        });
        if (!response.ok) {
            throw new ApiError(`Error getting experiment by name from tracking server: ${data.message || response.statusText}`, response.status);
        }
        return data.experiment;
    }
    /**
    ...
    async deleteExperiment(experiment_id) {
        const { response, data } = await apiRequest(this.trackingUri, 'experiments/delete', {
            method: 'POST',
            body: { experiment_id },
        });
        if (!response.ok) {
    ...
    async restoreExperiment(experiment_id) {
        const { response, data } = await apiRequest(this.trackingUri, 'experiments/restore', {
            method: 'POST',
            body: { experiment_id },
        });
        if (!response.ok) {
    ...
    async updateExperiment(experiment_id, new_name) {
        const { response, data } = await apiRequest(this.trackingUri, 'experiments/update', {
            method: 'POST',
    ...
    async setExperimentTag(experiment_id, key, value) {
        const { response, data } = await apiRequest(this.trackingUri, 'experiments/set-experiment-tag', {
            method: 'POST',
